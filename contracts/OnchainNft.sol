...
contract OnchainNft is ERC721 {
    ...
    */
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        string memory metaData = generateMetadata(_tokenId);
     ...
     * @return Returns a string that contains the metadata of a given image
    */
    function generateMetadata(uint256 _tokenId) private view returns(string memory) {
        return string(abi.encodePacked('[{"trait_type":"level", "value":"', HelperLibrary.toString(getLevel(numberToDisplay(_tokenId))+1), '"}]'));
    }
...
'<text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="',level_colors[getLevel(numberToDisplay(_tokenId))],'">',HelperLibrary.toString(numberToDisplay(_tokenId)),'</text>',
  '<style> text{font-size: 40pt; font-weight: 800;}</style></svg>'
    ...
    */
    function numberToDisplay(uint256 _tokenId) private view returns (uint256) {
        uint256 _numTodisplay = userBalance(_tokenId);

        //if the balance of the user is gretater than 100, return 100
        //else retun the balance
        return _numTodisplay > 100 ? 100 : _numTodisplay;
    }
